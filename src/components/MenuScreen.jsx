...
const PolyominoButton = ({ onClick, shape, color, glowColor, icon: Icon, iconSvg, title, subtitle }) => {
  ...
  return (
    <button 
      onClick={handleClick}
      className={`relative w-full p-4 ${color} rounded-xl hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 border border-white/20 touch-manipulation`}
...
const MenuScreen = ({ 
  ...
  return (
    <div 
      className="full-screen-scroll bg-slate-950"
      style={{
        position: 'fixed',
        inset: 0,
        overflow: 'auto',
    ...
    >
      {/* Fixed background elements - pointer-events-none so they don't block scrolling */}
      ...
      {/* Scrollable content */}
      <div 
        className="relative min-h-full flex flex-col items-center justify-center p-4"
        style={{
          minHeight: '100%',
