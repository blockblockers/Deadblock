...
const GameScreen = ({
  ...
  const dragCellRef = useRef(null); // Store current cell during drag
  ...
  const calculateTouchedPieceCell = useCallback((piece, touchX, touchY, elementRect, currentRotation, currentFlipped) => {
    ...
    // Calculate cell size in the piece preview
    const cellWidth = elementRect.width / pieceWidth;
    const cellHeight = elementRect.height / pieceHeight;
    
    // Find which cell was touched relative to piece bounds
    const relX = touchX - elementRect.left;
    const relY = touchY - elementRect.top;
    
    const touchedCol = Math.floor(relX / cellWidth) + minX;
    const touchedRow = Math.floor(relY / cellHeight) + minY;
    
    return { row: touchedRow, col: touchedCol };
  }, []);
  ...
  // End drag - set pendingMove from dragCellRef
  const endDrag = useCallback(() => {
    ...
    // Set pendingMove from dragCellRef/dragPreviewCell when drag ends
    const piece = draggedPiece || draggedPieceRef.current;
  ...
  }, [isDragging, draggedPiece, dragPreviewCell, rotation, flipped, setPendingMove, detachGlobalTouchHandlers]);

  // CRITICAL: Update refs SYNCHRONOUSLY (not in useEffect) to avoid race conditions
  updateDragRef.current = updateDrag;
  endDragRef.current = endDrag;

  // Create drag handlers for PieceTray - INLINE pattern to avoid timing issues
  const createDragHandlers = useCallback((piece) => {
    ...
    // Touch start - do all setup INLINE
    ...
    // Touch move/end - global handlers take care of it
    ...
    const handleMouseDown = (e) => {
      ...
      const touchedCell = calculateTouchedPieceCell(piece, e.clientX, e.clientY, elementRect, rotation, flipped);
      pieceCellOffsetRef.current = touchedCell;
      setPieceCellOffset(touchedCell);
      
      const offsetX = elementRect ? e.clientX - (elementRect.left + elementRect.width / 2) : 0;
      const offsetY = elementRect ? e.clientY - (elementRect.top + elementRect.height / 2) : 0;
  ...
  }, [gameOver, usedPieces, gameMode, currentPlayer, rotation, flipped, onSelectPiece, setPendingMove, calculateTouchedPieceCell, attachGlobalTouchHandlers]);
