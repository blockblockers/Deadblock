...
const OnlineMenu = ({ 
  ...
  const [showActiveInvites, setShowActiveInvites] = useState(false);
          ...
          {!loading && !profileError && (
            <>
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      onClick={() => {
                        soundManager.playClickSound?.('select');
                        setViewingPlayerId(profile?.id);
                        setViewingPlayerData(profile);
                      }}
                      className="w-12 h-12 rounded-full bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center text-white font-bold text-lg hover:scale-105 hover:shadow-[0_0_20px_rgba(251,191,36,0.5)] transition-all"
                      title="View Profile"
                    >
              ...
              <div className="bg-slate-800/40 rounded-xl p-4 mb-4 border border-slate-700/50">
                {/* Header with View Invites button */}
                <div className="flex items-center justify-between mb-2">
                  ...
                  {/* View Active Invites Button */}
                  {(receivedInvites.length > 0 || sentInvites.length > 0 || inviteLinks.length > 0) && (
                    <button
                      onClick={() => {
                        soundManager.playButtonClick();
                        setShowActiveInvites(true);
                      }}
                      className="px-2.5 py-1 bg-amber-500/20 text-amber-300 rounded-lg text-xs font-medium flex items-center gap-1.5 hover:bg-amber-500/30 transition-all border border-amber-500/30"
                    >
                      <Mail size={12} />
                      {receivedInvites.length + sentInvites.length + inviteLinks.length}
                    </button>
                  )}
                </div>
                            ...
                            >
                              <div className="flex items-center justify-between mb-2">
                                <span className="text-white text-sm font-medium">
                                  {invite.recipientName || invite.invitee?.username || 'Pending...'}
                                </span>
                                <button
          ...
          <div className="bg-slate-900 rounded-xl max-w-md w-full max-h-[80vh] overflow-hidden border border-amber-500/30 shadow-[0_0_50px_rgba(251,191,36,0.2)]">
            ...
            <div className="p-4 border-b border-amber-500/20 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-bold text-amber-300">Game Invites</h2>
              </div>
              <button
                onClick={() => setShowActiveInvites(false)}
                className="p-1 text-slate-400 hover:text-white transition-colors"
              >
                <X size={24} />
              </button>
            ...
            <div className="p-4 overflow-y-auto max-h-[60vh]" style={{ WebkitOverflowScrolling: 'touch' }}>
                    ...
                    {receivedInvites.map(invite => (
                      ...
                      >
                        ...
                        >
                          ...
                          <span className="text-white text-sm font-medium">{invite.from_user?.username || 'Unknown'}</span>
                        </button>
                        <div className="flex gap-2">
                          <button
                            onClick={() => handleDeclineInvite(invite)}
                          ...
                          <button
                            ...
                            disabled={processingInvite === invite.id}
                            className="px-3 py-1.5 bg-green-500 text-white rounded-lg text-xs font-medium hover:bg-green-400 transition-all"
                          >
                            Accept
                          </button>
                        </div>
                      ...
                      >
                        <button
                          onClick={() => {
                            soundManager.playClickSound?.('select');
                            setShowActiveInvites(false);
                            setViewingPlayerId(invite.to_user?.id);
                            setViewingPlayerData(invite.to_user);
                          }}
                          className="flex items-center gap-3 hover:opacity-80"
                        >
                          <div className="w-8 h-8 rounded-full bg-slate-600 flex items-center justify-center text-white text-xs font-bold">
                          ...
                          <span className="text-slate-300 text-sm">{invite.to_user?.username || 'Unknown'}</span>
                        </button>
                        <button
                          onClick={() => handleCancelInvite(invite)}
                          disabled={processingInvite === invite.id}
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {inviteLinks.map(invite => (
                      <div
                        key={invite.id}
                        className="p-3 bg-slate-800/50 rounded-lg border border-cyan-500/20"
                      >
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-white text-sm font-medium">
                            {invite.recipientName || invite.invitee?.username || 'Pending...'}
              ...
              {receivedInvites.length === 0 && sentInvites.length === 0 && inviteLinks.length === 0 && (
                <div className="text-center py-8">
                  <Mail className="mx-auto text-slate-600 mb-2" size={40} />
                  <p className="text-slate-400">No active invites</p>
                </div>
              )}
            </div>
          </div>
