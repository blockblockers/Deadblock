...
const DifficultySelector = ({ selectedDifficulty, onSelectDifficulty, onStartGame, onBack }) => {
  ...
  return (
    <div className="min-h-screen relative p-4 flex items-center justify-center overflow-hidden bg-slate-950">
      ...
      <div className="relative bg-slate-900/90 backdrop-blur-md rounded-2xl shadow-2xl p-6 sm:p-8 max-w-lg w-full border border-purple-500/30 shadow-[0_0_30px_rgba(168,85,247,0.3)]">
        {/* Header */}
            ...
            return (
              <button
                ...
                className={`w-full p-4 rounded-xl border transition-all duration-300 text-left relative overflow-hidden ${
                  isSelected 
                    ? `bg-gradient-to-r ${diff.color} border-white/30 shadow-[0_0_30px_${diff.glowColor}]`
        ...
        {selectedDifficulty === AI_DIFFICULTY.PROFESSIONAL && (
          <div className="mt-4 p-3 bg-purple-900/30 border border-purple-500/30 rounded-lg">
            <p className="text-xs text-purple-300">
              ...
              Expert mode uses Claude AI to analyze each move. Requires internet connection.
        ...
        <button 
          onClick={handleStart}
          className="w-full mt-6 p-4 bg-gradient-to-r from-cyan-500 to-blue-600 text-white rounded-xl font-bold tracking-wide hover:from-cyan-400 hover:to-blue-500 transition-all shadow-[0_0_25px_rgba(34,211,238,0.5)] flex items-center justify-center gap-2"
        >
          <Bot size={20} />
          START GAME
        </button>
