...
// v7.14: Add Friend search — auto-searches as user types (matches OnlineMenu challenge search
//        pattern). Replaced Enter-key/button search with onChange handler, added search icon
//        and inline spinner. Search results are annotated with friend/pending status.
...
const FriendsList = ({ userId, onInviteFriend, onSpectate, onViewProfile, onClose }) => {
  ...
  // Search for users
  // v7.10: Auto-search as user types (matches OnlineMenu challenge search pattern)
  const handleSearch = useCallback(async (query) => {
    ...
    try {
      const { data } = await inviteService.searchUsers(query, userId, 10);
      
      if (data) {
        // Annotate results with friend/pending status
      ...
      >
        ...
        >
          {loading ? (
            ...
            /* Add friends tab */
            <div>
              {/* v7.10: Auto-search input matching OnlineMenu challenge search pattern */}
              <div className="relative mb-4">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-10 py-2.5 bg-slate-800 border border-slate-600 rounded-lg text-white text-sm placeholder-slate-500 focus:border-amber-500 focus:outline-none"
